...
class DataDemoHandler (webapp2.RequestHandler):
    def post(self):
        mov_title = self.request.get('movie_title')
        runtime = int(self.request.get('movie_runtime'))
        user_rating= float(self.request.get('movie_rating'))
        my_movie = Movie(title= title, runtime_min = runtime, rating = rating)

class TestDataHandler(webapp2.RequestHandler):
    def get(self):
        movie_template = jinja_current_directory.get_template('/data_demo.html')
        self.response.write(movie_template.render())

    def post(self):
        mov_title = self.request.get('movie_title')
        runtime = int(self.request.get('movie_runtime'))
        user_rating= float(self.request.get('movie_rating'))
        my_movie = Movie(title= mov_title, runtime = runtime, rating = user_rating)
        my_movie.put()

    q = Movie.all()
    result = q.get
    q=Movie.all(keys_only=True)
    self.response.write(q)

app = webapp2.WSGIApplication([
('/fortune', FortuneHandler),
('/dataDemo', TestDataHandler),
], debug=True)
